...
Vec3f* dPunten = gpuMallocCopy(h_points);
u8* dMask=nullptr;  // alles actief
...
bool ok = hVindIdeaalFrame(dPunten, N, dMask, d_labels, p, frame, log);
if(!ok){
...
hRotatieNaarIdeaal(dPunten, N, frame);

hFineLabeling(dPunten, N, d_labels, frame, p);
