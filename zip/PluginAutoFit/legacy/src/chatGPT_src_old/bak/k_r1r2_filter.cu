...
__global__ void kR1r2Filter(const Vec3f*** restrict** d_points, int N, uint8_t*** restrict** d_maskTriangle,
                              float3*** restrict** d_normTriangle, int*** restrict** d_inlierSlab,
                              uint8_t*** restrict** dIsChaos)
{
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if(i >= N) return;

    const float r1      = Consts::R1Cluster;
    const float r2      = Consts::R2Ring;
    const float r2h     = Consts::R2HalfClaimSafe;
    ...
    dIsChaos[i]    = 0;
    d_inlierSlab[i] = 0;
    d_normTriangle[i]    = make_float3(0, 0, 0);
    ...
    if(slabCnt& lt; = Consts::SlabMinInliers - 1)
    {
        ...
        if(!d_maskTriangle[i]) dIsChaos[i] = 1;
    }
}