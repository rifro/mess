...
namespace hostVis
{
    ...
    static inline void printFrameNudgePreview(const Vec3f& asX, const Vec3f& asY, const Vec3f& asZ)
    {
        Vec3f Z{0, 0, 1};
        Vec3f v   = cross(Z, asZ);
        float c   = dot(Z, asZ);
        float ang = std::acos(std::clamp(c, -1.0f, 1.0f));
        Quat  qz  = (std::fabs(ang) < 1e-6f) ? Quat{1, 0, 0, 0} : quatFromAxisAngle(v, ang);

        Vec3f X{1, 0, 0};
        Vec3f Xp   = quatRotate(qz, X);
        Vec3f t    = cross(Xp, asX);
        float c2   = dot(Xp, asX);
        float ang2 = std::acos(std::clamp(c2, -1.0f, 1.0f));
        Quat  qx   = (std::fabs(ang2) < 1e-6f) ? Quat{1, 0, 0, 0} : quatFromAxisAngle(t, ang2);

        Quat q = quatMul(qx, qz);

        float yawDeg = 0.f, pitchDeg = 0.f;
        quatToZyAngles(q, yawDeg, pitchDeg);

        float clip = cfg::get().stemmen.clipKleinGraads;
        clipSmallDegrees(clip, yawDeg);
        clipSmallDegrees(clip, pitchDeg);

        float maxLock = cfg::get().stemmen.maxLockGraads;
        bool  reject  = (std::fabs(yawDeg) > maxLock || std::fabs(pitchDeg) > maxLock);

        std::cout << "[FrameCheck] Rotaties (Z,Y): yaw=" << yawDeg << "°, pitch=" << pitchDeg << "°";
        if(reject)
        {
...
static inline void logStatus(const StatBlok& s)
{
    std::cout << "hits+=" << s.hitsNieuw << " total=" << s.hitsTotaal << " near=" << s.nearCount << " ok=" << s.okCount
              << " slots=" << s.slotCount << " d_angle=" << std::fixed << std::setprecision(2) << s.d_angleDegrees << "°"
              << " varCos=" << std::setprecision(4) << s.varCos << " " << (s.locked ? "LOCKED" : "-") << std::endl;
}
...
// ------------------------------------------------------------
void hRunOrchestratieLoggingLoop()
{
    StatBlok acc;
    ...
    // Dummy loop just once for test
    {
        ...
        hostVis::printFrameNudgePreview(asX, asY, asZ);
    }
}
