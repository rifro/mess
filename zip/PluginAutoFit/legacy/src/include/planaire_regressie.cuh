...
namespace jbf
{
    ...
    dLineaireRegressieInVlak(const float3* __restrict__ points, const uint32_t* __restrict__ Index, uint32_t M,
                              const float3& vlakNorm, // u-of-v normaal voor dit doorsnedevlak
                              const float3& tang,      // tangent-richting binnen het projectievlak (u of v)
                              const float3& nCurrent) // huidige n (voor 3D lift)
    {
        // Projecteer naar lokaal 2D: x = dot(p,tang), y = dot(p, n_current × tang)
        float3 ortho = dNorm3(dCross3(nCurrent, tang));
        ...
        for(uint32_t i = 0; i < M; ++i)
        {
            ...
            float        x = jbf::dDot3(p, tang);
            float        y = jbf::dDot3(p, ortho);
            Sx += x;
            Sy += y;
            Sxy += x * y;
            Sxx += x * x;
...
#pragma unroll
    ...
    __device__ __forceinline__ float3 dPlanaireRegressie(const float3* __restrict__ points,
                                                          const uint32_t* __restrict__ Index, uint32_t M,
                                                          const float3& nIn, const float3& X, const float3& Y,
                                                          const float3& Z, float cosParallelMax, float maxRefineDeg)
    {
        float3 u, v;
        jbf::dBepaalUVUitN(nIn, X, Y, Z, cosParallelMax, u, v);

        // Regressie in U-doorsnede (vlak bepaald door n_in en u)
        auto ruU = dLineaireRegressieInVlak(points, Index, M, /*vlak_norm=*/nIn, /*tang=*/u, /*n=*/nIn);
        // Regressie in V-doorsnede
        auto ruV = dLineaireRegressieInVlak(points, Index, M, /*vlak_norm=*/nIn, /*tang=*/v, /*n=*/nIn);

        float3 nStar = jbf::dNorm3(jbf::dCross3(ruU.dir, ruV.dir));
        return jbf::dClampRichting(nIn, nStar, maxRefineDeg);
    }
