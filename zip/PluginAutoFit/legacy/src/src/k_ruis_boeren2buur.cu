...
__global__ void kInitLabels(uint8_t* __restrict__ labels, uint32_t N, uint8_t waarde)
{
...
__global__ void kRuisBoerenTweedeBuur(const float3* __restrict__ points, uint8_t* __restrict__ labels, uint32_t N,
                                       VoxelRaster vr, VoxelIndexing vx, float rKeep /*≈ 1–3 mm*/)
{
    const float r2 = rKeep * rKeep;
    uint32_t    v  = blockIdx.x; // 1 CTA per voxel (of chunk-wise)
    if(v >= vx.numVoxels) return;

    uint32_t begin = vx.dVoxelStarts[v];
    uint32_t eind  = vx.dVoxelStarts[v + 1];
        ...
        for(int zz = z0; zz <= z1 && found < 2; ++zz)
            for(int yy = y0; yy <= y1 && found < 2; ++yy)
                for(int xx = x0; xx <= x1 && found < 2; ++xx)
                {
                    ...
                    uint32_t nbB = vx.dVoxelStarts[nb];
                    uint32_t nbE = vx.dVoxelStarts[nb + 1];
