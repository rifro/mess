                                           ...
                                           RichtingPeak& peakOut)
{
    ...
    uint32_t* d_kept;
    float*    d_rms;
    cudaMalloc(&dNstar, sizeof(Vec3f));
    cudaMalloc(&d_kept, sizeof(uint32_t));
    cudaMalloc(&d_rms, sizeof(float));

    float trimFrac = ((float)k_trimFracInt) * 0.01f;
    kPlanairRegressieKernel<<<1, 1, 0>>>(d_pointsSorted, N, n0, trimFrac, dNstar, d_kept, d_rms);
    cudaDeviceSynchronize();

    Vec3f    h_nstar;
    uint32_t h_kept;
    float    hRms;
    cudaMemcpy(&h_nstar, dNstar, sizeof(Vec3f), cudaMemcpyDeviceToHost);
