...
#define CUDA_CHECK(expr)                                                                                               \
    ...
    {                                                                                                                  \
        ...
        void build_mortonOrder(DeviceCloud& dc, const float3* h_pos, const uint32_t* h_ccIndex, uint32_t N, float3 bbMin,
                              float meterToMm, ConstsStruct voxel, ConstsStruct sub, GpuRunLog* dLog /*optional*/)
        {
                                   ...
                                   GpuRunLog* dLog /*optional*/)
        {
            ...
            if(dLog)
            {
                CUDA_CHECK(cudaMemcpy(&dLog->downsampleOutCount, &outN, sizeof(uint32_t), cudaMemcpyHostToDevice));
                // mark flag via host memcpy (simple)
                bool ds = true;
                CUDA_CHECK(cudaMemcpy(&dLog->downsampled, &ds, sizeof(bool), cudaMemcpyHostToDevice));
            }
            return outN;
        }
        ...
        uint32_t runStageFindFrameAndDownsample(DeviceCloud& dc, const ConstsStruct& voxel, const ConstsStruct& sub,
                                                 ...
                                                 GpuRunLog*                       dLog /* device ptr */)
        {
            {
                ScopedTimer t("detectPlanes (stub)");
                detectPlanesKernel<<<1, 1>>>(dLog, existingFrameIsIdeal);
                CUDA_CHECK(cudaGetLastError());
            }
            {
                ...
                applyRotationKernel<<<grid, block>>>(thrust::rawPointerCast(dc.dPos.data()), N, dLog);
                CUDA_CHECK(cudaGetLastError());
            }
            uint32_t outN = 0;
            {
                ScopedTimer t("downsampleIndices");
                outN = downsampleIndices(dc, maxDownsample, d_downIndex, dLog);
            }
            return outN;
        }
