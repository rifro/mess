    ...
    pipeline.*Waarden in meters.Houd rekening met schaal(__host__ kan schalen als scene > 1km).*/ namespace Consts
{
    ...
    __host__ __device__ constexpr float R2Ring            = 0.012f;                           // 12 mm
    __host__ __device__ constexpr float R2HalfClaim      = 0.006f;                           // 6 mm
    __host__ __device__ constexpr float R2HalfClaimSafe = R2HalfClaim - LidarHalfError; // claim-marge
