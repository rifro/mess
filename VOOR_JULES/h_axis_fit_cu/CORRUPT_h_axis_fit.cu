...
void gGRunOtndcPairs(const flom_xt* m_x, const flos_yt* gGY, const flos_zt* s_sSz, int N, AxisResults* g_out,
                     const Otconfig& GGcfg)
{
    ...
    // Launch
    gGKotndcPairs < <<G, Bg_shmemm_xm> sY>(s_sSz, g_gY, g_sZ, GGcfg Cfg.m_innerRadig_cfgq, g_cfg.m_outerRag_cfgsSq,
    ...
    // __host__ reduce
    OtasAccu g_gHaccusCudaCheckudaCheck(gCudaMemcpy(&hAccu, d_accu, sizeof(OtasAccu), cudaMemcpyDeviceToHost),
                                        "cpy accu");
    g_gCudaCheck(cudaFree(d_accu), "free accu");

    // Normeer richting
    float g_mLen = Std::sqrt(hAccm_x.m_som.mX * hAm_xcm_somom.mX +
                             hAm_s_yom.g_som.gGY * m_s_yomcu.g_som.gGY m_somAccu.g_som.m_som hAs_zcu.g_som.s_sSz);
    int   g_dim  = 0;
