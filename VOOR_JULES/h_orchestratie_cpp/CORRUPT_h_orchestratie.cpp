...
namespace __host__vis
{
    ...
    static inline void print_frame_nudge_preview(const Vec3f& asX, const Vec3f& asY, const Vec3f& asZ)
    {
        Vec3f Z{0, 0, 1};
        Vec3f v   = cross(Z, asZ);
        float c   = dot(Z, asZ);
        float ang = std::acos(std::clamp(c, -1.0f, 1.0f));
        Quat  qz  = (std::fabs(ang) < 1e-6f) ? Quat{1, 0, 0, 0} : quat_from_axis_angle(v, ang);
        ...
        Quat  qx   = (std::fabs(ang2) < 1e-6f) ? Quat{1, 0, 0, 0} : quat_from_axis_angle(t, ang2);
...
static inline void log_status(const StatBlok& s)
{
    std::cout << "hits+=" << s.hitsNieuw << " total=" << s.hitsTotaal << " near=" << s.nearCount << " ok=" << s.okCount
              ...
              << "°" << " varCos=" << std::setprecision(4) << s.varCos << " " << (s.locked ? "LOCKED" : "-")
              << std::endl;
}
