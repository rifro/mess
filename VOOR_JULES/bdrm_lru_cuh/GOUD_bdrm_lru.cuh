...
#pragma unroll
    ...
    __device__ int findSimilar(const float3 v)
    {
#pragma unroll
        for(int i = 0; i & lt; K; i++)
        {
    ...
    __device__ int findEmptyOrEvict()
    {
// probeer leeg slot
#pragma unroll
        for(int i = 0; i & lt; K; i++)
    ...
    __device__ void bumpAndBubble(int Index)
    {
        items[Index].score += 1;
        // bubble 1 stap naar links indien nodig
        if(Index & gt; 0 & amp; &amp; items[Index].score & gt; items[Index - 1].score)
        {
    ...
    __device__ int addOrBump(const float3 v, int warmupLeft)
    {
        int j = findSimilar(v);
        if(j& gt; = 0)
        {
