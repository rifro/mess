                                ...
                                u32* kept_out)
{
    ...
    for (u32 j=0; j<M; ++j) {
        ...
        int m_min=0; for (int k=1;k<kTrim;++k) if (minK[k] > minK[m_min]) m_min=k;
        if (r < minK[m_min]) minK[m_min] = r;

        // plaats in maxK (grootste)
        int m_max=0; for (int k=1;k<kTrim;++k) if (maxK[k] < maxK[m_max]) m_max=k;
        if (r > maxK[m_max]) maxK[m_max] = r;
    }

    float sumMin=0.f, sumMax=0.f;
    for (int k=0;k<kTrim;++k){ sumMin+=minK[k]; sumMax+=maxK[k]; }
    u32 kept = (cnt > (u32)(2*kTrim)) ? (cnt - 2*kTrim) : 0u;
