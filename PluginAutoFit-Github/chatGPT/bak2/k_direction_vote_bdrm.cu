...
template <int K>
**global** void k_direction_vote_bdrm(const float3* **restrict** d_normalTriangle,
const u8* **restrict** d_OkForVote,
int N,
...
if (gid &lt; N &amp;&amp; d_OkForVote[gid]) {
    ...
    if (Index &gt;= 0) {
        ...
        for (int i=0;i&lt;K;i++){
            ...
            if (i==Index) continue;
            if (fabsf(dot3f(lru-&gt;items[i].dir, n)) &lt;= Consts::OrthoCosMax){
...
k_direction_vote_bdrm<Consts::LruSlots><<<grid,block,sh,stream>>>(
