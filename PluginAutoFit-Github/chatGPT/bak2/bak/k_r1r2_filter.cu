...
**global** void k_r1r2_filter(const Vec3f* **restrict** d_points,
int N,
u8* **restrict** d_maskTriangle,
float3*  **restrict** d_normalTriangle,
int*     **restrict** d_inlierSlab,
u8* **restrict** d_isChaos)
{
...
for (int j=0;j&lt;N;j++){
    ...
    if (d &lt; r1-Consts::LidarHalfError) continue;
    if (d &gt; r2+Consts::LidarHalfError) continue;
...
// init outputs
d_maskTriangle[i]=0; d_isChaos[i]=0; d_inlierSlab[i]=0; d_normalTriangle[i]=make_float3(0,0,0);
