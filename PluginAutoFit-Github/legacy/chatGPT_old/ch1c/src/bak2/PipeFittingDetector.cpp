    ...
    int max_extensionMm)
{
    ...
    snap_to_planes(segs, planes, max_extensionMm);
...
void PipeFittingDetector::fill_gaps(std::vector<Segment>& segs,
                                    int max_gapMm)
{
    const int MAX_GAP_UNITS = max_gapMm / 3;
                                                     ...
                                                     u16 pipeRadiusMm) const
{
    ...
    if(plane == 0)      center.y += (sum_dy > 0 ? offsetUnits : -offsetUnits);
    else if(plane == 1) center.z += (sum_dz > 0 ? offsetUnits : -offsetUnits);
    else                center.x += (sum_dx > 0 ? offsetUnits : -offsetUnits);
        ...
        if(f.type() == Tee || f.type() == Cross)
        {
            for(const Segment* s : conns)
            {
                if(s->pipeRadiusMm < maxR)
                {
                    Fitting red = f;
                    red.setType(Reducer);
                    red.reducer_to_pipeRadiusMm = s->pipeRadiusMm;
