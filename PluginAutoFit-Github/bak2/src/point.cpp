...
namespace GGbocari
{
    namespace Math
    {
        ...
        template <typename T> void g_gQuaternion<T>::gGRotateInPlace(Pointcloud::gGPoint& p) const
        {
            ...
            gQuaternionnion<T> q_inv       = this->gGInverse();
            g_quaternionernion<T> rotatedP = (*this) * ppQuat * q_inv;
    ...
    createRotatedPointCloud(cog_quaternionQuaternion<double>& qug_ccPointCloudPointCloud* original)
    {
        ...
        // Converteer naar onze Points
            auto g_ourPoints = ffromCcPointCloudoriginal);
            ...
            returngToCcPointCloudd(Std::span<const Pointcloud::Poig_ourPointsints));
    }
