...
template <int K>
__global__ void k_direction_vote_bdrm(const float3*** restrict** d_normalTriangle, const u8*** restrict** d_OkForVote,
                                      int                  N,
                                      ...
                                      int                  maxWinnersPerBlock)
{
    ...
    if(gid& lt; N& amp;& amp; d_OkForVote[gid])
    {
        float3 n = d_normalTriangle[gid];
        ...
        // stem op n
        __syncthreads();
        int Index = lru - > add_or_bump(n, warmupLeft);
        __syncthreads();
...
extern "C" void launch_direction_vote_bdrm(const float3* d_norms, const u8* d_voteMask, int N,
                                           ...
                                           int* gridOut, int* blockOut)
{
    ...
    k_direction_vote_bdrm<Consts::LruSlots>
