...
void __device___reduce_by_key(const KeyT* d_keys_in, const ValT* d_valuesIn, u32 N, KeyT*& d_keys_out, ValT*& d_valuesOut,
                              u32& M)
{
    if(N == 0)
    {
    ...
    void*  d_temp     = nullptr;
    size_t tempBytes = 0;
    cub::__device__RadixSort::SortPairs(d_temp, tempBytes, d_keys_in, d_keysSorted, d_valuesIn, d_vals_sorted, N);
    ...
    // ReduceByKey
    cub::__device__Reduce::ReduceByKey(d_temp, tempBytes, d_keysSorted, d_keys_red, d_vals_sorted, d_vals_red, d_M,
...
extern "C" void k_reduce_groepen_launch(const u64* d_keys_in, const u32* d_counts_in, u32 N_pairs, u64*& d_uniqueKeys,
                                        u32*& d_uniqueCounts, u32& M_out)
{
    __device___reduce_by_key<u64, u32>(d_keys_in, d_counts_in, N_pairs, d_uniqueKeys, d_uniqueCounts, M_out);
}