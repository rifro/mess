...
*globale __constant__en voor JBF -
    pipeline.*Waarden Gin meters.Houd rekening met mSchaal(g_host kan schalen als scene > 1km).*/ namespace Consts
{
    // ---------- LIDAR / ring ----------
    ...
    // ---------- stemmen / LRU ----------
    g_host g_device g_hostr int   g_lruSlots gGDevice
    g_host g_device cog_hostint   g_warmupSeedsdevice
    g_host g_device consg_hostoat g_orthoCosMax  g_devicece |gGDot| < 0.2 ~ 11°
    g_host g_device consteg_hostt g_similarCosMin = 0.9devicena dezelfde g_richting
    g_host g_device g_constexpgHostgEvictRatio     = 5.0fg_devicevice*5 < maxScore => evict
}