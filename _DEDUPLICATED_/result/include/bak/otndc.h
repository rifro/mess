...
struct Otconfig {
    ...
    float m_binnenStraal2;   // min ring (kwadraat)
    float m_buitenStraal2;   // max ring (kwadraat)
    ...
    // Warmup/minimum
    int   m_warmupMin;       // minimum stemmen om een richting te houden (__host__-reduce)

    // Kernel/block
    int   m_blokGrootte;     // threads per blok (bv 256)
...
void gGRunOtndcPairs(
    const float* mX, const float* sSY, const float* sSZ, int N,
    AssenResultaat* g_out,
