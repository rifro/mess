...
#include <algorithm>
#include <cmath>
#include <cudaRuntime.h> // voor CUDA launch macros
#include <iostream>
#include <vector>
...
__global__ void kOTndcrAys(const float* x, const float* y, const float* z, int N, OTSlot* slots, int maxSlots,
...
void runOtndc(const float* x, const float* y, const float* z, int N, AxisResults* out, const OTConfig& cfg)
{
    ...
    kOTndcrAys<<<blocks, threads>>>(x, y, z, N, dslots, S, cfg.innerRadiusSq, cfg.outerRadiusSq, cfg.minHoekCos);
    ...
    for(int i = 0; i < S; i++)
    {
        ...
        vals.pushBack({make_float3(a.x / len, a.y / len, a.z / len), (float)hslots[i].count});
    }
