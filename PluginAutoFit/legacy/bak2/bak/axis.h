...
#include <chrono> // De cruciale header
    ...
    template <typename RotationAccessorFunctor, typename OtherAccessorFunctor> float rotateAndAnalyzeCoarseMiddleFine()
    {
        ...
        std::ostringstream ssCoarse;
        ssCoarse << "Phase: Coarse, Best Angle: " << bestCoarseAngle;
        dbg(ssCoarse.str().cStr());
...
private:
                              ...
                              unsigned numStrips)
    {
        ...
        {
            float bestVariance = -std::numericLimits<float>::infinity();
        ...
        using ZAxis       = Axis<ZAccessorFunctor>;
        ...
        for(; angle <= stopAngle; angle += stepDeg)
        {
            ...
            Plane<RotationAccessorFunctor, OtherAccessorFunctor> planeRotAxisOtherAxis(rotationAxis1, otherAxis1);

            // Plane 2: Rotatie-as en de Z-as
            RotationAxis                                     rotationAxis2(bbMin, bbMax, numStrips, rotPixels);
            ZAxis                                           zAxis2(bbMin, bbMax, numStrips, zPixels);
            Plane<RotationAccessorFunctor, ZAccessorFunctor> planeRotAxisZAxis(rotationAxis2, zAxis2);

            // Plane 3: OtherAxis and the Z-axis
            OtherAxis                                     otherAxis3(bbMin, bbMax, numStrips, otherPixels);
            ZAxis                                        zAxis3(bbMin, bbMax, numStrips, zPixels);
            ...
            if(!workingCloud)
            {
                ...
                dbg(ss.str().cStr()); // Roep dbg hier al aan bij een fatale fout
                return 0.0f;
            }
            ...
            for(unsigned i = 0; i < workingCloud->size(); ++i)
            {
                ...
                planeRotAxisOtherAxis.addPoint(*pt);
                planeRotAxisZAxis.addPoint(*pt);
                planeOtherAxisZAxis.addPoint(*pt);
            }

            // Bereken de totale variantie score voor de huidige hoek
            auto score = planeRotAxisOtherAxis.calcTotalVariance() + planeRotAxisZAxis.calcTotalVariance() +
            ...
            logAndSavePlane(planeRotAxisOtherAxis, phaseName, sort, angle, score, imageSavePath);
            logAndSavePlane(planeRotAxisZAxis, phaseName, sort, angle, score, imageSavePath);
