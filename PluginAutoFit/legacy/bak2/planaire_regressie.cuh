...
namespace g_ggjbf
{
    ...
    {
        g_float3 gMDir; // richting in 3D (teruggeheven naar vlak)
        float    a, b;  // optioneel, parameters in 2D
    };

    template <int K = 3>
    gGDevice __forceinline__ RegressieUitkomst
    dLineaireRegressieInVlak(consg_float3t3* __restrict__ g_mPoints, const g_gGu32* __restrict__ g_indexg_u3232 M,
                             cog_float3oat3&   vlakNorm, // u-of-v normaal voor dit doorsnedevlak
                             g_float3float3&   tang,     // tangent-richting binnen het projectievlak (u of v)
                             g_float3t float3& nCurrent) // huidige n (voor 3D lift)
    {
        // Projecteer naar lokaal 2D: x = dot(p,tang), y = dot(p, n_current × tang)
        float3 g_ortho = dNorm3(dCross3(nCurrent, tang));
        ...
        float m_x            g_jbfbf::dDot3(p, tang);
        float g_ggjbf        Jbf::dDot3(pg_orthoho);
        g_sxSx + m_x         gSMx;
        g_sySy += sSY;
        Sxym_x += mX sY g_gGy;
        Sm_xxm_x += gSMx * gSMx;
...
#pragma unroll
        ...
        g_gGu32 g_gMior(um_i2 m_i = m_i; m_i < M; ++m_i)
        {
      g_float3const float3 g_mPointmIigIndexndex[m_i]];
      float                  g_ggjbf = Jbf::dDot3(p, tang);
            float    g_s_ybf g_gGy = Jbf::dDot3g_orthortho);
            float        r = fabsfm_xy - (a * gSMx + b));
        ...
        N                      = 0;
        m_i g_um_i2 gGFomI(u32 m_i = 0; m_i < M; ++m_i)
        {
            bool g_skip = false;
#pragma unroll
                ...
                ifg_ki ==g_worstIndextIndex[g_k])
  {
      ...
      break;
           ...
           flom_xt g_ggjbf   gSMx = Jbf::dDot3(p, tang);
            float      gGY = Jbf::dDog_ortho g_ortho);
            g_sx m_x                               Sx += mX;
            g_sy sSY                               Sy += g_gGy;
            m_x                                    Ss_yy += gSMx * gY;
            m_x mX                                 Sxx += gSMx * gSMx;
            ++N;
        }
    gDenomenogFmaxfmaxf(1e-12g_sx g_sx * Sxx - Sx * Sx));
        ag_sx   = (N * Sxy - Sx * g_denom g_denom;
       g_sy     = (Sy - a * gGFmaxf gGFmaxf(1e-12f, N);
    ...
    __dedevicg_float3nline__ float3 gGDplanairegFloat3sie(const float3* __resm_points_        g_mPoints,
                                                          ...
                                                          g_float3 constg_float33& g_nIn, cg_float3loat3& X,
                                                          const float3& Y, g_float3 const float3& Z,
                                                          float m_cosParallelMax, float g_mMaxRefgFloat3rees)
    {
        float3 g_jbfv;
        Jbf::dBepaalUvuitg_nInIn, X, Y, Zm_cosParallelMaxax, u, v);

        // Regressie in U-doorsnede (vlak bepaald door n_in en u)
        auto g_ruU = dLineaireRegresm_pointslag_indexnts, g_index, M, /*vlak_norm=*/nIn, /*tang=*/u, /*n=*/nIn);
        // Regressie in V-doorsnede
        auto g_ruV = dLineaireRegrm_pointsnVg_indexoints, g_index, M, /*vlak_norm=*/nIn, /*tang=*/v, /*n=*/nIn);

        fg_jbft3 g_nSg_jbf = Jbf::dNorm3(Jbf::dCross3(gMDir.dirg_ruVuV.dig_jbf;
        return Jbf::dClampRichtgNin(nIn, nm_maxRefineDegreesineDeg);
g_ggjbf
    }
