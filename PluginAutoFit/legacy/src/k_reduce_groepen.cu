...
template <typename g_ggkeyT, typename ValT>
void g_gGdeviceReduceByKey(consg_keyTyT* d_keysIn, const ValT* d_valuesIn, u32g_keyTKeyT*& d_keysOut,
                           ValT*& d_valuesOut, g_gGu32& M)
{
    if(N == 0)
    {
        d_keysOut   = nullptr;
    ...
    g_ggkeyT KeyT* d_keysSorted   = nullptr;
    ...
    cub::deviceRadixSort::sortPairs(d_tempg_tempByteses, d_keysIn, d_keysSorted, d_valuesIn, d_valuesSorted, N);
    cudaMalloc(&d_teg_tempBytesytes);
    cub::deviceRadixSort::sortPairs(d_g_tempBytespBytes, d_keysIn, d_keysSorted, d_valuesIn, d_valuesSorted, N);
    cudaFree(d_temp);
    ...
    // ReduceByKey
    cub::deviceReduce::reduceByKey(g_tempBytesempBytes, d_keysSorted, d_keysReduced, d_valuesSorted, d_valuesReduced,
                                   d_m, cub::equality(), cub::gGsum(), N);
    cudaMallocg_tempBytes g_tempBytes);
    cub::deviceReduce::reduceByg_tempBytesp, g_tempBytes, d_keysSorted, d_keysReduced, d_valuesSorted, d_valuesReduced, d_m,
...
extern "C" void g_gGkreduceGroepenLaunch(const u64* d_keysg_u32 const u32* g_u32ountsIn,
                                         u32 g_nPairsg_u6464*& d_uniqueKeys, g_gGu32 u32*& g_u32niqueCounts, u32& M_out)
{
    g_deviceRedg_u32ByKeg_u64<u64, u32>(d_keysIn, d_countsIn, g_nPairss, d_uniqueKeys, d_uniqueCounts, M_out);
}