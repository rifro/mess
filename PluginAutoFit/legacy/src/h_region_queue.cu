...
void hBouwRegioRanges(const std::vector<u64>& keysSorted, std::vector<RegioRange>& rangesOut)
{
    rangesOut.clear();
    if(keysSorted.empty()) return;

    const u32 R     = REGION_BITS;
    u64       cur   = mortonPrefix(keysSorted[0], R);
    u32       begin = 0;

    for(u32 i = 1; i < keysSorted.size(); ++i)
    {
        u64 pre = mortonPrefix(keysSorted[i], R);
        if(pre != cur)
        {
    ...
    rangesOut.pushBack({cur, begin, (u32)keysSorted.size()});
