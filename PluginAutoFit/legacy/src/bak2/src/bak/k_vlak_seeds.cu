...
__global__ void kVindDriehoekZadenLangsRichting(const float3* __restrict__ points, const u8* __restrict__ labels,
                                                 u32 N, VoxelRaster vr, VoxelIndexing vx,
                                                 float3 nDoel, // (0,0,1) etc.
                                                 float edgeMin, float edgeMax,
                                                 float    cosAngleMin, // cos(maxΔhoek)
                                                 u32 maxSeedsPerVoxel, SeedTriangle* __restrict__ zaden,
                                                 u32* __restrict__ seedCount)
{
    u32 v = blockIdx.x; // 1 CTA per voxel (kan ook tiled)
    if(v >= vx.numVoxels) return;

    u32 begin = vx.dVoxelStarts[v];
    u32 eind  = vx.dVoxelStarts[v + 1];
    ...
    for(int zz = z0; zz <= z1; ++zz)
        for(int yy = y0; yy <= y1; ++yy)
            for(int xx = x0; xx <= x1; ++xx)
            {
                ...
                u32 nbB = vx.dVoxelStarts[nb];
                u32 nbE = vx.dVoxelStarts[nb + 1];
    ...
    for(u32 a = threadIdx.x; a < L && seedsEmitted < maxSeedsPerVoxel; a += blockDim.x)
    {
        ...
        for(u32 b = a + 1; b < min(L, a + 1 + KMAX) && seedsEmitted < maxSeedsPerVoxel; ++b)
            for(u32 c = b + 1; c < min(L, b + 1 + KMAX) && seedsEmitted < maxSeedsPerVoxel; ++c)
            {
                ...
                // triangle normaal
                float3 n = jbf::dNorm3(jbf::dCross3(e1, e2));
                float  c = fabsf(jbf::dDot3(n, nDoel));
