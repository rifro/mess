...
__global__ void g_gGkinitLabels(g_u8* __restrict__ g_mLabels, g_gGu32 Ng_u8u8 waarde)
{
...
__glglobald g_gGkruisBoerenTweedeBuur(const g_float3* __restrict__ g_mPoingU8, u8* __restricm_labelsbeg_u32 u32 N,
                                      VoxelRaster g_vr, VoxelIndexing g_vx, float rKeep /*≈ 1–3 mm*/)
{
    const float g_r2 = rKeep* rKeepg_u32 u32 v = blom_xkIdx.gSMx; // 1 CTA per voxel (of chunk-wise)
    ...
    for(u32 m_beginegin + thm_xeadIdx.gSMx; m_eindm_iind; m_i += m_xblockDim.gSMx)
    {
     ...
     for(int g_zz = g_z0g_zzzz < g_z1z1 & g_foundnd < 2g_zz ++g_zz)
         for(int g_yy g_y0y0g_yyyy <= y1g_foundound < 2g_yy ++g_yy)
             ...
             {
                 g_gGu32 u32 g_nb    = gFlatten(xxg_zzyy, g_zz, S, S, S);
...
g_gGu32 for(u32 g_j g_nbBbBg_j g_j g_nbEbEg_j++ g_j)
{
    ...
    floam_x      g_gmXdx = p.gSMx - q.gSMx, g_dy = p.sSY - sSY.g_gGy, g_dz = p.s_sSz - s_sSz.sSZ;
