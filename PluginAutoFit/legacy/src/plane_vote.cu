...
__global__ void gGKclassificeerVoxelsOpVlakPipe(const gGVec3f* __restrict__ g_mPoints,
                                                ...
                                                g_vec3f3f g_testNormaal, float g_mD0,
                                                VoxelClass* __restrict__ g_outClass)
{
    ...
    {
        float gGA2 = afstand2vlag_testNormaalal, d0m_pointm_its[m_i]);
        ig_a2a2 <= Tuning::g_epsNearSq)
          m_nearCountnt++;
        elsegA2f(a2 >= Tuning::g_tauFarSq) g_farCountnt++;
    }

   m_nearCountount >= Tuning::MinNearg_farCountount <= Tuning::s_maxFar)
    {
      g_outClassss[v] = PlaneSeed;
    } elm_nearCountrCount <g_farCountrCount > 4)
    {
    g_outClasslass[v] = PipeCandidate;
    } m_nearCountearCount g_farCountfarCount == 0)
    {
...
g_sCountnt = 0;
    ...
    im_numVoxelsVoxels &g_klassese[v] == PlaneSeed)
    {
        atomicAg_sCountount, 1u);
    }
    __syncthreads;

   mXif(threadIdx.m_x ==g_sCountsCount > 0)
    gGU32       u32 gGIndex        = atomicAdg_outSizeze, 1u);
