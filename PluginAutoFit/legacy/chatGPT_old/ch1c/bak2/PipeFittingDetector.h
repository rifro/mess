...
struct Fitting
{
    ...
    u16       reducerToPipeRadiusMm=0; // bij Reducer: doel-straal
...
private:
    ...
    void snapToPlanes(std::vector<Segment>& segs,
                        std::span<const Plane> planes,
                        int maxExtMm);

    void mergeOverlapping(std::vector<Segment>& segs);

    void fillGaps(std::vector<Segment>& segs,
                   int maxGapFillMm);

    void clusterNodes(const std::vector<Segment>& segs,
                       std::vector<Fitting>& fittings);

    u8 determineElbowPlane(u8 axis1, u8 axis2) const noexcept;

    Point3mm calculateTorusCenter(const Segment* s1,
                                    const Segment* s2,
