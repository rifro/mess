    ...
    int maxExtensionMm)
{
    ...
    clusterNodes(segs, fittings);

    return fittings;
}

void PipeFittingDetector::snapToPlanes(std::vector<Segment>& segs,
                                         std::span<const Plane> planes,
                                         int maxExtMm)
{
...
void PipeFittingDetector::mergeOverlapping(std::vector<Segment>& segs)
{
...
void PipeFittingDetector::fillGaps(std::vector<Segment>& segs,
                                    int maxGapMm)
{
    const int MAX_GAP_UNITS = maxGapMm / 3;
    ...
    for(u8 axis=0; axis<3; ++axis)
    {
        ...
        for(size_t i=0; i+1<line.size(); ++i)
        {
            ...
            if(manhattanGap > 10 && manhattanGap <= MAX_GAP_UNITS)
            {
                ...
                gap.a = s1->b;
                gap.b = s2->a;
                gap.pipeRadiusMm = s1->pipeRadiusMm; // zelfde diameter als hoofd
                gap.axis = axis;
...
u8 PipeFittingDetector::determineElbowPlane(u8 axis1, u8 axis2) const noexcept
{
    // 0+1→2 (XZ), 0+2→1 (XY), 1+2→0 (YZ)
    return static_cast<u8>(3 - (axis1 + axis2));
}

Point3mm PipeFittingDetector::calculateTorusCenter(const Segment* s1,
                                                     const Segment* s2,
                                                     u16 pipeRadiusMm) const
{
    ...
    u8 plane = determineElbowPlane(s1->axis, s2->axis);

    // Buitenste points (uiteinden van de segments)
    Point3mm outer1 = (s1->a == node ? s1->b : s1->a);
    Point3mm outer2 = (s2->a == node ? s2->b : s2->a);

    i32 sumDx = outer1.x + outer2.x - 2 * node.x;
    i32 sumDy = outer1.y + outer2.y - 2 * node.y;
    i32 sumDz = outer1.z + outer2.z - 2 * node.z;

    // Ruwe schatting torusStraal: pipeStraal + marge
    i32 offsetUnits = static_cast<i32>((pipeRadiusMm + 44) / 3);

    Point3mm center = node;
    if(plane == 0)      center.y += (sumDy > 0 ? offsetUnits : -offsetUnits);
    else if(plane == 1) center.z += (sumDz > 0 ? offsetUnits : -offsetUnits);
    else                center.x += (sumDx > 0 ? offsetUnits : -offsetUnits);
...
void PipeFittingDetector::clusterNodes(const std::vector<Segment>& segs,
                                        std::vector<Fitting>& fittings)
{
    // tolerantie 12mm → 4 grid units → 4^2 = 16
    constexpr i32 MaxDriftSq = 16;

    struct Cluster
    {
        Point3mm avg{};
        int      count = 0;
        std::vector<Point3mm> points;
    ...
    for(const auto& kv : nodeMap)
    {
        ...
        u16 maxR = 0;
        u16 minR = std::numericLimits<u16>::max();
        bool anyHyp = false;

        for(const Segment* s : conns)
        {
            maxR = std::max(maxR, s->pipeRadiusMm);
            minR = std::min(minR, s->pipeRadiusMm);
        ...
        f.directionCount = static_cast<u8>(std::min<size_t>(conns.size(), 4));
        u8 axisMask = 0;
        for(size_t i=0; i<f.directionCount; ++i)
        {
            ...
            f.packedDirections |= static_cast<u16>(packed) << (i*3);
            axisMask |= static_cast<u8>(1u << axis);
        }

        // Type bepalen obv aantal richtingen en axes
        int axisCount = builtinPopcount(axisMask);

        if(conns.size() == 2 && axisCount == 2)
        {
            f.setType(Elbow);
            f.elbowPlane = determineElbowPlane(conns[0]->axis, conns[1]->axis);
            f.elbowCenter = calculateTorusCenter(conns[0], conns[1], maxR);
            f.torusRadiusMm = static_cast<u16>(maxR + 50);
            f.elbowCorner = f.pos;
        }
        ...
        if(f.type() == Tee || f.type() == Cross)
        {
            for(const Segment* s : conns)
            {
                if(s->pipeRadiusMm < maxR)
                {
                    Fitting red = f;
                    red.setType(Reducer);
                    red.reducerToPipeRadiusMm = s->pipeRadiusMm;
